...
export default function UserFollowersFilter() {
  // Initialize form
  ...
  const toggleFilterEnabled = () => {
    const currentValue = form.getValues("discovery_filter_user_followers_enabled");
    form.setValue("discovery_filter_user_followers_enabled", !currentValue);
            ...
            <Card>
              <CardContent className="p-4 pt-6">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h3 className="text-base font-medium">Filter by Minimum User Followers</h3>
                    <p className="text-sm text-muted-foreground">
                      Only show collaborations from users with at least this many Twitter followers
                    </p>
                  </div>
                  <Switch
                    checked={watchedValues.discovery_filter_user_followers_enabled}
                    onCheckedChange={toggleFilterEnabled}
                    aria-label="Enable user followers filter"
                  />
                </div>
                        ...
                        >
                          {TWITTER_FOLLOWER_COUNTS.map((count) => (
                            <div key={count} className="flex items-center space-x-2">
                              <RadioGroupItem value={count} id={`userCount-${count}`} />
                              <Label htmlFor={`userCount-${count}`} className="font-normal">
                                {count}
                              </Label>
                            </div>
                          ))}
                ...
                {watchedValues.discovery_filter_user_followers_enabled && 
                 watchedValues.twitter_followers && (
                  <div className="mt-4 p-3 bg-primary/5 rounded-lg border border-primary/10">
                    <p className="text-sm">
                      Only showing collaborations from users with at least{" "}
                      <span className="font-medium">{watchedValues.twitter_followers}</span> Twitter followers
                    </p>
                  </div>
                )}
              </CardContent>
