...
import { COLLAB_TYPES, type MarketingPreferences } from "@/../../shared/schema";
...
export default function CollabTypesFilter() {
  // Initialize form
  ...
  const toggleFilterEnabled = () => {
    const currentValue = form.getValues("discovery_filter_collab_types_enabled");
    form.setValue("discovery_filter_collab_types_enabled", !currentValue);
    ...
    <MobileCheck>
      <div className="container pb-6 min-h-screen">
        <PageHeader
          ...
          subtitle="Filter by collaboration format"
          backUrl="/filters"
        />
            ...
            <Card>
              <CardContent className="p-4 pt-6">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h3 className="text-base font-medium">Filter by Collaboration Types</h3>
                    <p className="text-sm text-muted-foreground">
                      Only show collaborations matching these types
                    </p>
                  </div>
                  <Switch
                    checked={watchedValues.discovery_filter_collab_types_enabled}
                    onCheckedChange={toggleFilterEnabled}
                    aria-label="Enable collaboration types filter"
                  />
                </div>
                      ...
                      <FormItem className="space-y-4">
                        <div className="grid grid-cols-1 gap-3">
                          {COLLAB_TYPES.map((collabType) => (
                            <div key={collabType} className="flex items-center space-x-2">
                              <Checkbox
                                id={`collabType-${collabType}`}
                                checked={field.value?.includes(collabType)}
                                onCheckedChange={(checked) => {
                                  const updatedValue = checked
                                    ? [...field.value, collabType]
                                    : field.value.filter(
                                        (value: string) => value !== collabType
                                      );
                                  field.onChange(updatedValue);
                                }}
                              />
                              <Label
                                htmlFor={`collabType-${collabType}`}
                ...
                {watchedValues.discovery_filter_collab_types_enabled && 
                 watchedValues.collabs_to_discover.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-sm font-medium mb-2">Selected Types:</h4>
                    <div className="flex flex-wrap gap-2">
                      {watchedValues.collabs_to_discover.map((type) => (
                        <Badge 
                          key={type} 
                          variant="outline"
