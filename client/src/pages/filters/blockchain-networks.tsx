...
export default function BlockchainNetworksFilter() {
  ...
  useEffect(() => {
    if (marketingPrefs && !isLoading) {
      ...
      // Set initially expanded categories based on selected networks
  ...
  const toggleFilterEnabled = () => {
    const currentValue = form.getValues("discovery_filter_blockchain_networks_enabled");
    form.setValue("discovery_filter_blockchain_networks_enabled", !currentValue);
    ...
    <MobileCheck>
      <div className="container pb-6 min-h-screen">
        <PageHeader
          ...
          subtitle="Filter by blockchain networks"
          backUrl="/filters"
        />
          ...
          <form className="space-y-4 pb-24">
            <Card>
              <CardContent className="p-4 pt-6">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Only show collaborations from companies operating on these networks
                    </p>
                  </div>
                  <Switch
                    checked={watchedValues.discovery_filter_blockchain_networks_enabled}
                    onCheckedChange={toggleFilterEnabled}
                    aria-label="Enable blockchain networks filter"
                  />
                </div>
                ...
                {watchedValues.discovery_filter_blockchain_networks_enabled && 
                 watchedValues.company_blockchain_networks.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-sm font-medium mb-2">Selected Networks:</h4>
                    <div className="flex flex-wrap gap-2">
                      {watchedValues.company_blockchain_networks.map((network) => (
                        <Badge 
                          key={network} 
                          variant="outline"
