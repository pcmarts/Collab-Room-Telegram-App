...
export default function EditCollaborationSteps({ id }: EditCollaborationProps = {}) {
  ...
  const form = useForm<CreateCollaboration>({
    ...
    defaultValues: {
      ...
      specific_date: new Date().toISOString().split('T')[0], // Current date in YYYY-MM-DD
      topics: [],
      is_free_collab: true, // Always set to true to enforce free collabs
  ...
  useEffect(() => {
    async function fetchCollaborationData() {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`/api/collaborations/get/${id}`);
        ...
        console.log("Fetched collaboration data:", collabData);
        
        // Set the selected collab type first
        setSelectedCollabType(collabData.collab_type);
        
        // Populate form with existing data
        form.reset({
          title: collabData.title || "Collaboration",
          description: collabData.description || "Created using Collab Room",
          collab_type: collabData.collab_type,
          date_type: collabData.date_type || "specific_date",
          specific_date: collabData.specific_date ? collabData.specific_date.split('T')[0] : new Date().toISOString().split('T')[0],
          topics: collabData.topics || [],
          is_free_collab: true, // Always true as per requirements
          
          // Filtering criteria
          filter_company_sectors_enabled: collabData.filter_company_sectors_enabled || false,
          filter_company_followers_enabled: collabData.filter_company_followers_enabled || false,
          filter_user_followers_enabled: collabData.filter_user_followers_enabled || false,
          filter_funding_stages_enabled: collabData.filter_funding_stages_enabled || false,
          filter_token_status_enabled: collabData.filter_token_status_enabled || false,
          filter_blockchain_networks_enabled: collabData.filter_blockchain_networks_enabled || false,
          
          required_company_sectors: collabData.required_company_sectors || [],
          required_funding_stages: collabData.required_funding_stages || [],
          required_token_status: collabData.required_token_status || false,
          required_blockchain_networks: collabData.required_blockchain_networks || [],
          min_company_followers: collabData.min_company_followers || TWITTER_FOLLOWER_COUNTS[0],
          min_user_followers: collabData.min_user_followers || TWITTER_FOLLOWER_COUNTS[0],
          
          // Type-specific details
          details: collabData.details || {}
        });
      } catch (error) {
        console.error("Error fetching collaboration:", error);
        toast({
          title: "Error",
          description: error instanceof Error ? error.message : "An error occurred while fetching the collaboration",
          variant: "destructive"
        });
        // Redirect on error
        setLocation('/marketing-collabs-new');
  ...
  const steps = [
    { 
      ...
      description: "Edit the type of collaboration" 
    },
    { 
      title: "Collaboration Details", 
      description: "Edit the specific details for your collaboration" 
    },
    { 
      title: "Matching Requirements", 
      description: "Edit criteria for who can apply to your collaboration" 
    }
  ];
  ...
  const nextStep = () => {
    ...
    if (currentStep === 0) {
      ...
      if (collabType === "Podcast Guest Appearance") {
        const podcastDetails = details as { podcast_name: string; short_description: string; podcast_link: string };
        if (!podcastDetails.podcast_name?.trim()) {
          canProceed = false;
          errorMessage = "Podcast name is required";
        } else if (!podcastDetails.short_description?.trim()) {
          canProceed = false;
          errorMessage = "Short description is required";
        }
      }
      else if (collabType === "Twitter Spaces Guest") {
        const twitterDetails = details as { twitter_handle: string; host_follower_count: string };
        if (!twitterDetails.twitter_handle?.trim()) {
          canProceed = false;
          errorMessage = "Twitter handle is required";
        } else if (!twitterDetails.twitter_handle.startsWith("https://x.com/")) {
          canProceed = false;
        ...
        } else if (!coMarketingDetails.host_twitter_handle?.trim()) {
          canProceed = false;
          errorMessage = "Twitter handle is required";
        } else if (!coMarketingDetails.host_twitter_handle.startsWith("https://x.com/")) {
          canProceed = false;
          errorMessage = "Twitter handle must start with https://x.com/";
        }
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Use PATCH to update existing collaboration
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder="Select estimated reach" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
        ...
        return (
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Only receive applications from companies in specific sectors
                  </p>
                </div>
                <FormField
                  control={form.control}
                  name="filter_company_sectors_enabled"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                      <FormControl>
                        <Switch
                ...
                <div className="pl-2 border-l-2 border-gray-200">
                  {Object.entries(COMPANY_TAG_CATEGORIES).map(([category, tags]) => (
                    <div key={category} className="mb-6">
                      ...
                      >
                        <div className={`transform transition-transform ${expandedCategories.includes(category) ? 'rotate-90' : ''}`}>▶</div>
                        <h4 className="font-medium ml-2 text-sm">{category}</h4>
                      </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium">Company Follower Count</h3>
                  <p className="text-sm text-muted-foreground">
                    Only receive applications from companies with minimum follower counts
                  </p>
                </div>
                <FormField
                  control={form.control}
                  name="filter_company_followers_enabled"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                      <FormControl>
                        <Switch
              ...
              {form.watch('filter_company_followers_enabled') && (
                <div className="pl-4 border-l-2 border-gray-200">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Minimum Company Followers</FormLabel>
                        <Select
                          value={field.value}
                          onValueChange={field.onChange}
                        >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium">User Follower Count</h3>
                  <p className="text-sm text-muted-foreground">
                    Only receive applications from users with minimum follower counts
                  </p>
                </div>
                <FormField
                  control={form.control}
                  name="filter_user_followers_enabled"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                      <FormControl>
                        <Switch
              ...
              {form.watch('filter_user_followers_enabled') && (
                <div className="pl-4 border-l-2 border-gray-200">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Minimum User Followers</FormLabel>
                        <Select
                          value={field.value}
                          onValueChange={field.onChange}
                        >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Only receive applications from companies at specific funding stages
                  </p>
                </div>
                <FormField
                  control={form.control}
                  name="filter_funding_stages_enabled"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                      <FormControl>
                        <Switch
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Only receive applications from companies with tokens
                  </p>
                </div>
                <FormField
                  control={form.control}
                  name="filter_token_status_enabled"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                      <FormControl>
                        <Switch
              ...
              {form.watch('filter_token_status_enabled') && (
                <div className="pl-4 border-l-2 border-gray-200">
                  <FormField
                    control={form.control}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Only receive applications from companies on specific networks
                  </p>
                </div>
                <FormField
                  control={form.control}
                  name="filter_blockchain_networks_enabled"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                      <FormControl>
                        <Switch
  ...
  return (
    <div>
      <PageHeader 
        ...
        subtitle="Update your collaboration details"
        backUrl="/marketing-collabs-new?tab=my"
      />
