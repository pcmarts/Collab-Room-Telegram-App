...
export default function CompanyDetails() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sectorData = JSON.parse(
        sessionStorage.getItem("companySectorData") || "{}",
      );
      ...
      const handle =
        ...
        (telegramData?.id
          ? `user_${telegramData.id.toString().substring(0, 8)}`
          : undefined);
      ...
      if (
        ...
        !basicData.website ||
        !basicData.job_title ||
        !basicData.twitter_url ||
        !basicData.funding_stage
      ) {
      ...
      const submitData = {
        ...
        twitter_handle: basicData.twitter_url
          .replace("https://x.com/", "")
          .replace("@", ""),
  ...
  const clearCategorySelections = (category: string) => {
    const networks = BLOCKCHAIN_NETWORK_CATEGORIES[
      category
    ] as readonly string[];
    setFormData((prev) => ({
      ...prev,
      blockchain_networks: prev.blockchain_networks.filter(
